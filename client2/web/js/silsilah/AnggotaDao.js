    ...
    (function (sl) {
        class AnggotaDao {
            async bacaAnak(anggota) {
                ...
                let h = await ha.comp.Util.Ajax2('post', sl.RouterKOns.server + ha.comp.Util.getUrl(sl.RouterKOns.p_anggota_id_anak_baca, [anggota.id]), '');
                return JSON.parse(h);
            }
            async bacaPasangan(anggota) {
                console.group('baca pasangan api:');
                let pas = [];
                let url = sl.RouterKOns.server + ha.comp.Util.getUrl(sl.RouterKOns.p_anggota_id_pas_lihat, [anggota.id]);
